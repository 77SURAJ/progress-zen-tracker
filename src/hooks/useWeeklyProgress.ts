    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (dailyError) {
          ...
          throw new Error(`Failed to fetch daily progress: ${dailyError.message}`);
        }
        ...
        throw new Error(error instanceof Error ? error.message : "Failed to load weekly data. Please try again later.");
      }
    },
  });
};